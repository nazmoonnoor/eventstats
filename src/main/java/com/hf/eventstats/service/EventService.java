...
import com.hf.eventstats.model.DataEvent;
...
public class EventService {
    ...
    private final int offset = 600000;

    public EventService() {
        dataEvent = new DataEvent();
        dataEvent.setCountX(new int[offset]);
        dataEvent.setCountY(new int[offset]);
        dataEvent.setSumX(new double[offset]);
        dataEvent.setSumY(new long[offset]);
        dataEvent.setCountX(new int[offset]);
    ...
    public synchronized void updateData(List<Event> events, Timestamp now) {
        ...
        if (diffPrevUpdate > offset ) {
            ...
            Integer lastValid = offset - diffPrevUpdate - 1; // Index of last isValid second
        ...
        events.forEach(event -> {
            ...
            if (diff >= 0 && diff < getOffset()){ // More than 60 secs diffs events are discarded. Negative diff are events with a future timeStamp which should occur.
